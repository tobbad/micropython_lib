...
class TestJsonSerDes(unittest.TestCase):
    ...
    def setUp(self):
        self.serdes =[ SerDes(version=1), SerDes(version=2)]
    ...
    def test_req2j_no_data(self):
        methodName = "Blabla"
        for i in range(len(self.serdes)):
            req_id, res = self.serdes[i].req_to_data("Blabla")
    ...
    def test_req2j_id_increment(self):
        methodName = "Blabla"
        for i in range(len(self.serdes)):
            req_id, res = self.serdes[i].req_to_data(methodName)
            jres=json.loads(res)
            self.assertEqual(jres['id'],0)
    ...
    def test_req2j_one_int(self):
        ...
        value = 1
        for i in range(len(self.serdes)):
            req_id, res = self.serdes[i].req_to_data(methodName, value)
            self.assertIsInstance(res, str)
            jres=json.loads(res)
            if i==1:
                self.assertEqual(jres['jsonrpc'],"2.0")
    ...
    def test_req2j_object(self):
        ...
        inst = simple()
        for i in range(len(self.serdes)):
            res = self.serdes[i].resp_to_data(inst)
    ...
    def test_rsp2j_reponse_and_error(self):
        for i in range(len(self.serdes)):
            res = self.serdes[i].resp_to_data(0, 1 ,1)
            self.assertIsNone(res)

    def test_rsp2j_reponse(self):
        for i in range(len(self.serdes)):
            res = self.serdes[i].resp_to_data(0, 1)
            self.assertIsInstance(res, str)
            jres=json.loads(res)
    ...
    def test_rsp2j_not_valid_error(self):
        for i in range(len(self.serdes)):
            res = self.serdes[i].resp_to_data(0, error=1)
            self.assertIsNone(res)

    def test_rsp2j_missing_code_field(self):
        err={'message':0}
        for i in range(len(self.serdes)):
            res = self.serdes[i].resp_to_data(0, error=err)
            self.assertIsNone(res)
    ...
    def test_rsp2j_ok(self):
        ...
        for i in range(len(self.serdes)):
            res = self.serdes[i].resp_to_data(id_res, error=err)
            self.assertIsInstance(res, str)
            jres=json.loads(res)
    ...
    def test_j2req_nojson(self):
        ...
        for i in range(len(self.serdes)):
            id_res, method, param = self.serdes[i].data_to_req(data)
    ...
    def test_j2req_wronjsonrpc(self):
        ...
        jdata2=jdata1.copy()
        jdata2['jsonrpc']='1.0'
        data=( json.dumps(jdata1), json.dumps(jdata2))
        for i in range(len(self.serdes)):
            id_res, method, param = self.serdes[i].data_to_req(data[i])
            self.assertIsNone(id_res)
    ...
    def test_j2req_Method_and_param(self):
        methodName='print'
        exp_params=[1,2,"42"]
        jdata1={'method':methodName, 'params':exp_params}
        jdata2=jdata1.copy()
        jdata2['jsonrpc']='2.0'
        data=( json.dumps(jdata1), json.dumps(jdata2))
        for i in range(len(self.serdes)):
    ...
    def test_j2req_Method_and_param_and_id(self):
        ...
        exp_params=[1,2,"42"]
        id_exp = 56
        jdata1={'method':methodName, 'params':exp_params, 'id':id_exp}
        jdata2=jdata1.copy()
        jdata2['jsonrpc']='2.0'
        data=( json.dumps(jdata1), json.dumps(jdata2))
        for i in range(len(self.serdes)):
    ...
    def test_j2resp_nojson(self):
        ...
        data=json.dumps(jdata)
        for i in range(len(self.serdes)):
            id_res, result, error = self.serdes[i].data_to_resp(data)
            self.assertIsNone(id_res)
    ...
    def test_j2resp_wrong_json(self):
        ...
        data=( json.dumps(jdata1), json.dumps(jdata2))
        for i in range(len(self.serdes)):
            id_res, result, error = self.serdes[i].data_to_resp(data[i])
            self.assertIsNone(id_res)
    ...
    def test_j2resp_jsonrpc_id_error_and_result_both_Not_Null(self):
        exp_id=1
        jdata1={'id':exp_id,'error':1, 'result':1}
        jdata2=jdata1.copy()
        jdata2['jsonrpc']='2.0'
        data=( json.dumps(jdata1), json.dumps(jdata2))
        for i in range(len(self.serdes)):
    ...
    def test_j2resp_jsonrpc_and_id_error_and_No_result(self):
        exp_id=1
        jdata1={'id':exp_id,'error':1, 'result':None}
        jdata2=jdata1.copy()
        jdata2['jsonrpc']='2.0'
        data=( json.dumps(jdata1), json.dumps(jdata2))
        for i in range(len(self.serdes)):
    ...
    def test_j2resp_jsonrpc_id_error(self):
        exp_id=1
        exp_error=1
        jdata1={ 'id':exp_id, 'error':exp_error}
        jdata2=jdata1.copy()
        jdata2['jsonrpc']='2.0'
        jdata1['result']=None
        data=( json.dumps(jdata1), json.dumps(jdata2))
        for i in range(len(self.serdes)):
            id_res, result, error = self.serdes[i].data_to_resp(data[i])
            self.assertEqual(exp_id, id_res, "JSON %d" % (i+1))
    ...
    def test_j2resp_jsonrpc_id_result(self):
        exp_id=1
        exp_result=1
        jdata1={'id':exp_id,'result':exp_result}
        jdata2=jdata1.copy()
        jdata2['jsonrpc']='2.0'
        jdata1['error']=None
        data=( json.dumps(jdata1), json.dumps(jdata2))
