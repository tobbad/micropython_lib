...
def getter(register_address):
    ...
    def decorator(func):
        def wrapper(self):
            return func(self, self.read_u8(register_address))
        return wrapper
    return decorator
...
def setter(register_address):
    ...
    def decorator(func):
        def wrapper(self, val):
            return self.write_u8(register_address, func(self, val))
        return wrapper
    return decorator
...
class SX127X(COM_SPI, multibyte):
    ...
    def get_mode(self):
        ...
        :return:    New mode
        """
        self.mode = self.read_u8(REG.LORA.OP_MODE)
        return self.mode
    ...
    def set_mode(self, mode):
        ...
        if mode == self.mode:
            return mode
        if self.DEBUG:
            print(MODE.lookup, mode)
            print("Mode <- %s (%d)\n" % (MODE.lookup[mode & self.__mode_mask], mode))
        self.mode = mode
        return self.write_u8(REG.LORA.OP_MODE, mode)
    ...
    def write_payload(self, payload):
        ...
        for byte in payload:
            self.write_u8(REG.LORA.FIFO, byte)
        return payload
    ...
    def read_payload(self , nocheck = False):
        ...
        payload = bytearray(rx_nb_bytes)
        for i in range(rx_nb_bytes):
            payload[i] = self.read(REG.LORA.FIFO)
        return payload
    ...
    def get_freq(self):
        ...
        :rtype:     float
        """
        f = self.read_u24_r(REG.LORA.FR_MSB)
        return f / 16384.
    ...
    def set_ocp_trim(self, I_mA):
        ...
        ocp_on = self.read_u8(REG.LORA.OCP) >> 5 & 0x01
        if I_mA <= 120:
    ...
    def get_fifo_addr_ptr(self):
        return self.read_u8(REG.LORA.FIFO_ADDR_PTR)

    def set_fifo_addr_ptr(self, ptr):
        return self.write_u8(REG.LORA.FIFO_ADDR_PTR, ptr)

    def get_fifo_tx_base_addr(self):
        return self.read_u8(REG.LORA.FIFO_TX_BASE_ADDR)
    ...
    def get_rx_nb_bytes(self):
        return self.read_u8(REG.LORA.RX_NB_BYTES)

    def get_rx_header_cnt(self):
        val = self.read_u16_r(REG.LORA.RX_HEADER_CNT_MSB)
        return val
    ...
    def get_symb_timeout(self):
        ...
        value = self.read_u16_r(SYMB_TIMEOUT_MSB)    # the MSB bits are stored in REG.LORA.MODEM_CONFIG_2
        value &= 0x03FF
        return value

    def set_symb_timeout(self, timeout):
        bkup_reg_modem_config_2 = self.read_u8(REG.LORA.MODEM_CONFIG_2)
        ...
        old_msb = self.read_u8(REG.LORA.MODEM_CONFIG_2) & 0x03
        old_lsb = self.read_u8(REG.LORA.SYMB_TIMEOUT_LSB)
        self.write_u8(REG.LORA.MODEM_CONFIG_2 , reg_modem_config_2) & 0x03
        self.write_u8(REG.LORA.SYMB_TIMEOUT_LSB, lsb)
        return old_lsb + 256 * old_msb

    def get_preamble(self):
        val = self.read_u16_r(REG.LORA.PREAMBLE_MSB)
        return val

    def set_preamble(self, preamble):
        old_preamble = self.read_u16_r(REG.LORA.PREAMBLE_MSB)
        old_msb, old_lsb = self.write_16_r(REG.LORA.PREAMBLE_MSB, preamble)
        return old_preamble
    ...
    def get_fei(self):
        freq_error = self.read_u24_r(REG.LORA.FEI_MSB)
        return freq_error
